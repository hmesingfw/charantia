...
*   option
*/
export function reqData(url, form, reqType, option = { isAlert: true }) {
    return new Promise((resolve) => {
        this.$http[reqType](url, form).then(res => {
            if (res.data.code == 200) {
                option.isAlert && this.$message.success(res.data.message);
                resolve(true)
            } else {
                option.isAlert && this.$message.info(res.data.message);
                resolve(false)
            }
        }).catch(err => {
            resolve(false)
