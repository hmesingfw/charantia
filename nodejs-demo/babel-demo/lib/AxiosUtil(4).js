...
axios.interceptors.response.use(function (response) {
  ...
  if (code == 401) {
    error.message = AxiosUtil.codeJson[code];
    setTimeout(function () {
      KT.logout(); // window.parent.location.href = '/web-catt-admin/app/login/login.html'
    }, 1000);
  }
...
Vue.directive('has-permission', {
  bind: function bind(el, binding, vnode) {
    var permissionList = sessionStorage.getItem(location.host) || []; //获取按钮权限,并转换成permissionList
    ...
    var config = {
      ...
      success: function success(data) {
        var btnRights = [];
        steamroller(data.data, btnRights);
        sessionStorage.setItem(location.host, btnRights);
        handleBtnEle(btnRights);
      }
    };

    if (permissionList.length > 0) {
      handleBtnEle(permissionList);
    } else {
    ...
    function handleBtnEle(btnRights) {
      if (!btnRights.includes(binding.value)) {
