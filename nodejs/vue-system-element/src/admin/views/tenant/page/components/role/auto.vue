<template>
    <dialog-model v-model="value" width="500" title="角色权限配置" @submit="handleRoleUser" :loading-button="loadingButton" @colse="colse" :isColse="false" @changeLoadingButton="loadingButton = false">
        <div class="role-list">
            <div class="all-check">
...
export default {
    ...
    methods: {
        ...
        init() {
            ...
            this.$http.get(this.$api.sys.menu, { params: param }).then(res => {
        ...
        async handleRoleUser() {
            ...
            let map = this.rml.map(id => ({
                roleId: this.row.id, menuId: id
            }))
            if (map.length > 0) {
                let data = await this.$http.put(`${this.$api.sys.roleMenu}`, map);
                this.$message.success(data.data.message);
            } else {
                let data = await this.$http.delete(`${this.$api.sys.roleMenu}/${this.row.id}`);
                this.$message.success(data.data.message);
            }
