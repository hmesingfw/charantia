...
export default {
    tempData: new Uint8Array(), // 取每次选取的并集
    preData: new Uint8Array(), // 撤销
    ...
    floodFill(image, px, py, colorThreshold, mask) {
        var c, x, newY, el, xr, xl, dy, dyl, dyr, checkY;
        var data = image.data;
        var w = image.width;
        var h = image.height;
        var bytes = image.bytes; // number of bytes in the color
        var maxX = -1;
        var minX = w + 1;
        var maxY = -1;
        var minY = h + 1;
        var i = py * w + px; // start point index in the mask data
        ...
        var preData = new Uint8Array();
        // 为了实现选取叠加而添加的代码部分
        if (this.tempData.length == 0) {
            this.tempData = result;
        } else {
            preData = new Uint8Array(this.tempData);// 保存上一步
            for (let i = 0; i < this.tempData.length; i++) { // 区域累加
                if (result[i] == 1 && this.tempData[i] != 1) {
                    this.tempData[i] = 1;
                }
            }
        }
    ...
    getBorderIndices(mask) {
        this.tempData = mask.data;// 确保tempData永远都是当前选取的数据
        var x; var y; var k; var k1; var k2;
        var w = mask.width;
        var h = mask.height;
        var data = mask.data;
        var border = []; // only border points
        var x1 = w - 1;
        var y1 = h - 1;
    ...
    getCurrentResult(mask) {
        if (!mask) return;
        this.tempData = mask.data;// 确保tempData永远都是当前的选取及填充数据
        var x; var y; var k;
        var w = mask.width;
        var h = mask.height;
        var data = mask.data;
        var res = []; // only res points
        var x1 = w - 1;
        var y1 = h - 1;
    ...
    clearPreData() {
        this.preData = new Uint8Array();
        this.tempData = new Uint8Array();
    }
